...
float4x4    mWorld          : register(c4);			// World mat
				...
				float4 tangent:	 TANGENT )
{
	...
	float4 wave_vec = frac(	wave_directions_in_X*texCoord.x +
				wave_directions_in_Y*texCoord.y + waveSpeed*fTime );
	...
	Out.pos = mul(mWorldViewProj, wave_vec);
	...
	warpedNormal = normalize(mul(mWorld, warpedNormal));
	
	// compute a normalized view vector
	float4 viewVector = normalize(vViewPosition - mul(mWorld, wave_vec));
